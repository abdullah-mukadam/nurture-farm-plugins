    ...
    rules: [
      ...
      },
      {
        ...
        exclude: [
          ...
          /nurture-farm-plugins.*\/node_modules\//,
          /\.test.jsx?$/,
        ],
        include: [
          new RegExp(`${APP_DIR}/src`),
          /superset-ui.*\/src/,
          /nurture-farm-plugins.*\/src/,
      ...
      {
        test: /\.css$/,
        include: [APP_DIR, /superset-ui.+\/src/, /nurture-farm-plugins.+\/src/],
        use: [
...
if (isDevMode) {
  ...
  Object.entries(packageConfig.dependencies).forEach(([pkg, version]) => {
    ...
    if (
      (/superset-ui/.test(pkg) || /nurture-farm-plugins/.test(pkg)) &&
